 ...
 * Contact.jsx — Fixed
 *
 * FIX #2: EmailJS credentials moved to environment variables.
...
const MagneticField = ({ label, children, required }) => {
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      >
        {label}
        {required && <span style={{ color: 'var(--accent)', marginLeft: '2px' }} aria-hidden="true">*</span>}
      </label>
      <div
...
const inputStyle = {
  ...
  color: 'var(--text-1)',
  borderRadius: '10px',
  cursor: 'text', // FIX: text cursor visible in inputs
};
...
const Contact = () => {
  ...
  useEffect(() => {
    /* FIX #2: init uses env var, not hardcoded key */
    ...
    <div style={{ background: 'var(--bg-0)', minHeight: '100vh', color: 'var(--text-1)' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '8rem 2.5rem 6rem' }}>
        ...
        >
          ...
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '1rem', color: 'var(--text-3)', maxWidth: '480px', lineHeight: 1.7 }}>
        ...
        {/* Dev warning when env vars missing */}
        {!emailjsConfigured && (
          <div
            role="alert"
            style={{
          ...
          >
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem', color: '#fbbf24' }}>
              ⚠️ EmailJS environment variables not configured.
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '4rem', alignItems: 'start' }}>
          ...
          {/* Right — Info panels (unchanged from original) */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            >
              ...
              {[
                { label: 'Email', value: 'kaustubhvdeshmukh2001@gmail.com', link: 'mailto:kaustubhvdeshmukh2001@gmail.com' },
                { label: 'Institution', value: 'R.H. Sapat College, Nashik', link: null },
                { label: 'Location', value: 'Nashik, Maharashtra, India', link: null },
              ].map(({ label, value, link }) => (
                <div key={label} style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start', padding: '0.75rem 0', borderBottom: '1px solid var(--border)' }}>
                  <div>
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.62rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: '0.25rem' }}>{label}</div>
                    {link ? (
                      <a href={link} style={{ fontFamily: 'var(--font-body)', fontSize: '0.82rem', color: 'var(--text-2)', textDecoration: 'none', wordBreak: 'break-all' }}
                        onMouseEnter={(e) => (e.currentTarget.style.color = 'var(--accent)')}
                        onMouseLeave={(e) => (e.currentTarget.style.color = 'var(--text-2)')}
                      >{value}</a>
                    ) : (
                      <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.82rem', color: 'var(--text-2)' }}>{value}</span>
                    )}
                  </div>
                </div>
              ))}
            ...
            <motion.div
              ...
              style={{ padding: '1.25rem 1.5rem', borderRadius: '12px', border: '1px solid rgba(129,140,248,0.2)', background: 'rgba(129,140,248,0.04)', display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}
            >
              ...
              <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.82rem', color: 'var(--text-3)', lineHeight: 1.6 }}>
                Typical response within <span style={{ color: 'var(--text-1)' }}>24–48 hours</span>.
